...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  // line_numbers list If it doesn't exist, add a new entry to the outgut

  int found = 0;
  for (int i = 0; i < *result_count && found == 0; i++) {
    Output op = output[i];
    if (op.doubled_value == input->doubled_value) {
      op.line_numbers[op.count] = input->line_number;
  ...
  if (found == 0) {
    Output new;
    new.doubled_value = input->doubled_value;
    new.line_numbers[0] = input->line_number;
    new.count = 1;
    output[*result_count] = new;
