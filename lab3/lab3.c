...
int main() {
  ...
  while (true) {
    ...
    buff = NULL; // so that getline finds a new address to allocate the new
                 // input preventing all the addresses in the array to be the
                 // same and thus all values being the same
  }
...
void updatehistory(char **arr, char *input) {
  ...
  if (current_index == INPUT_HISTORY_LENGTH) {
    current_index--;
